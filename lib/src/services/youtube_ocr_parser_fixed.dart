...
/// YouTube視聴履歴OCRデータの解析と分類システム（固定版）
class YouTubeOCRParser {
  ...
  static List<VideoData> parseOCRText(String ocrText) {
    ...
    // 2. 段落単位で分割（空行で区切られた動画ブロック）
    final videoBlocks = _splitIntoVideoBlocks(lines);
  ...
  /// 空行で区切られた動画ブロックに分割
  static List<List<String>> _splitIntoVideoBlocks(List<String> lines) {
  ...
  static VideoData? _parseVideoBlock(List<String> block) {
    ...
    if (lastLine.contains('・') && lastLine.contains('万回視聴')) {
      ...
      title = _buildTitle([block[0]]);
      
      // 2行目以降からチャンネル名と視聴回数を探す
      for (int i = 1; i < block.length; i++) {
        final line = block[i];
        
        if (_isViewCount(line)) {
          viewCount = line;
          // 前の行がチャンネル名の可能性
          if (i > 1 && channel == null) {
            channel = block[i - 1];
          ...
          break;
        } else if (channel == null && _isChannelName(line)) {
          channel = line;
        }
      }
