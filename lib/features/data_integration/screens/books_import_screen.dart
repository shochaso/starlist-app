...
class BooksImportScreen extends ConsumerStatefulWidget {
  final String serviceId;
  final String serviceName;
  final Color serviceColor;
  final IconData serviceIcon;

  const BooksImportScreen({
    super.key,
  ...
  @override
  ConsumerState<BooksImportScreen> createState() => _BooksImportScreenState();
}

class _BooksImportScreenState extends ConsumerState<BooksImportScreen> {
  String selectedImportType = 'reading_history';
  ...
  final Map<String, List<Map<String, String>>> importTypesByService = {
    'amazon_books': [
      {'id': 'reading_history', 'title': '読書履歴', 'subtitle': '読んだ本・書籍'},
      {'id': 'library', 'title': 'ライブラリ', 'subtitle': '所有・購入書籍'},
      {'id': 'wishlist', 'title': '欲しい物リスト', 'subtitle': '読みたい本'},
      {'id': 'reviews', 'title': 'レビュー', 'subtitle': '書籍レビュー・評価'},
    ],
    'kindle': [
      {'id': 'library', 'title': 'Kindleライブラリ', 'subtitle': '所有電子書籍'},
      {'id': 'reading_history', 'title': '読書履歴', 'subtitle': '読んだ本・進捗'},
      {'id': 'highlights', 'title': 'ハイライト', 'subtitle': 'メモ・マーカー'},
      {'id': 'notes', 'title': 'ノート', 'subtitle': '読書メモ・感想'},
    ],
    'rakuten_books': [
    ...
    'audible': [
      {'id': 'listening_history', 'title': '聴取履歴', 'subtitle': '聴いたオーディオブック'},
      {'id': 'library', 'title': 'ライブラリ', 'subtitle': '所有オーディオブック'},
      {'id': 'wishlist', 'title': '欲しい物リスト', 'subtitle': '聴きたい本'},
      {'id': 'reviews', 'title': 'レビュー', 'subtitle': 'オーディオブックレビュー'},
    ],
  };
  ...
  void _processData() async {
    ...
    try {
      ...
      if (parsedContent.isEmpty) {
        _showErrorSnackBar('書籍データを検出できませんでした。\nフォーマットを確認してください。');
      }
    } catch (e) {
      setState(() {
        isProcessing = false;
  ...
  List<Map<String, dynamic>> _parseContent(String text) {
    ...
    for (final line in lines) {
      ...
      if (trimmedLine.contains('タイトル:') ||
          ...
          trimmedLine.contains('書籍:')) {
        currentTitle = trimmedLine
            .replaceAll(RegExp(r'タイトル:|本:|書籍:'), '')
            .trim();
      }
      // 著者パターン
      else if (trimmedLine.contains('著者:') ||
          trimmedLine.contains('作者:')) {
        currentAuthor = trimmedLine
            .replaceAll(RegExp(r'著者:|作者:'), '')
            .trim();
      }
      // 日付パターン
      else if (trimmedLine.contains('読了日:') ||
          trimmedLine.contains('購入日:') ||
          trimmedLine.contains('日付:')) {
        currentDate = trimmedLine
            .replaceAll(RegExp(r'読了日:|購入日:|日付:'), '')
            .trim();
      }
      // 評価パターン
      else if (trimmedLine.contains('評価:') ||
          trimmedLine.contains('レビュー:')) {
        currentRating = trimmedLine;
      }
      // シンプルなタイトル（コロンがない場合）
      ...
      // アイテム作成
      if (currentTitle != null && (currentDate != null || items.length < 5)) {
        items.add({
          'title': currentTitle,
          'author': currentAuthor ?? '著者不明',
          'date': currentDate ?? '${widget.serviceName}で購入・読了',
  ...
  String _getServiceDescription() {
    switch (widget.serviceId) {
      case 'amazon_books':
        return 'Amazon Booksの購入履歴・読書履歴を取り込み';
      case 'kindle':
        return 'Kindleライブラリ・読書履歴を取り込み';
      case 'rakuten_books':
        return '楽天ブックスの購入履歴・電子書籍データを取り込み';
      case 'audible':
        return 'Audibleの聴取履歴・オーディオブックライブラリを取り込み';
      default:
        return '書籍・読書履歴データを取り込み';
    }
  }
        ...
        children: [
          Row(
            children: [
              ...
              Text(
                '検出された書籍 (${extractedContent.length}件)',
                style: TextStyle(
                  color: isDark ? Colors.white : Colors.black87,
                  fontSize: 16,
                  fontWeight: FontWeight.w600,
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 12),
          Text(
            '${processedContent.length}件の書籍データが正常に取り込まれました。',
            style: TextStyle(
...
またはAmazon Booksの購入履歴をOCRで読み取ったテキストをペーストしてください''';
      case 'kindle':
        return '''例：
タイトル: サンプル書籍
著者: 著者名
...
またはKindleライブラリの画面をOCRで読み取ったテキストをペーストしてください''';
      case 'rakuten_books':
        return '''例：
タイトル: サンプル書籍
著者: 著者名
...
または楽天ブックスの購入履歴をOCRで読み取ったテキストをペーストしてください''';
      case 'audible':
        return '''例：
タイトル: サンプルオーディオブック
著者: 著者名
...
またはAudibleのライブラリ画面をOCRで読み取ったテキストをペーストしてください''';
      default:
        return '''例：
タイトル: サンプル書籍
著者: 著者名
