...
| 価格入力 | スライダー＋テキスト入力。成人: 推奨 1,000〜5,000円 / 未成年: 推奨 500〜1,500円（初期値はスターが設定）。下限 100円、上限 10,000円。整数値のみ、税別表示。 |
...
- **UIスライダー**: 100円刻みでドラッグ可能、リアルタイム価格表示
- **端数処理**: 整数値のみ（小数点以下は四捨五入せず入力拒否）
- **税表示**: 価格入力欄は税別、決済確認画面で税込金額を表示
...
- **RLS側判定**: Supabase RLSで `age_category` を参照し、未成年価格上限（1,500円）を超える場合アクセス拒否
