...
```json
{
  "version": "2025-11-08",
  "tiers": {
    "light": {"student": 100, "adult": 980},
    "standard": {"student": 200, "adult": 1980},
    "premium": {"student": 500, "adult": 2980}
  },
  "limits": {
    "student": {"min": 100, "max": 1000},
    "adult": {"min": 980, "max": 100000},
    "step": 10,
    "currency": "JPY",
    "tax_inclusive": true
  }
...
**補足**: `limits` はカテゴリ全体の最低／最高値を示し、プラン別の厳密なレンジ（下記表）や推奨価格は UI/バリデーション層で `pricing_cards.dart` や `pricing_validator.dart` に沿って適用される。

## 価格仕様 v1（MVP）

| 区分 | プラン | 推奨価格（おすすめ表示） | 最小価格 | 最大価格 | 刻み | 備考 |
| --- | --- | --- | --- | --- | --- | --- |
| **未成年（student）** | Light | 100円 | 100円 | 300円 | 10円 | 保護者負担感を考慮し、ハード上限1,000円 |
| | Standard | 200円 | 300円 | 500円 | 10円 | 保護者同意が前提 |
| | Premium | 500円 | 500円 | 1,000円 | 10円 | 限られたプレミアム体験 |
| **成人（adult）** | Light | 980円 | 980円 | 30,000円 | 10円 | MVPでは質と柔軟性の両立 |
| | Standard | 1,980円 | 1,980円 | 50,000円 | 10円 | 主力プラン |
| | Premium | 2,980円 | 2,980円 | 100,000円 | 10円 | ハイエンドプラン / 10万円上限 |

価格表にあるすべての金額は税込表示であり、Flutter UI では表内の「推奨価格」を「おすすめ」バッジで強調、最小/最大は `pricing_validator.dart` で検証されます。

## スーパーチャット（投げ銭）ティア v1

スーパーチャットは `super_chat_pricing` の `min_amount`/`max_amount` と各 `tier_n_threshold` を活用し、以下のティアで特典（ピン表示・表示時間・バッジ）を切り替える:
...
上限は 100,000円（10万円）で、Supabase バリデーションと UI でこれを超える入力を拒否します。未成年スターについては 1,000円以下のティアを推奨し、投げ銭 UI に保護者同意注意文を併設します。
