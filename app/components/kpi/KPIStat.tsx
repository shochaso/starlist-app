...
interface KPIStatProps {
  ...
  value: number | string;
  hint?: string;
  verdict?: KPIVerdict; // μ±2σ: warning, μ±3σ: fail, otherwise: pass
  mean?: number;
  stdDev?: number;
  currentValue?: number;
}
...
export default function KPIStat({ label, value, hint, verdict, mean, stdDev, currentValue }: KPIStatProps) {
  const borderColor = getVerdictColor(verdict);
  const badge = getVerdictBadge(verdict);
  
  // Calculate verdict from mean ± stdDev if not provided
