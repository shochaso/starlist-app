...
serve(async (req) => {
  ...
  try {
    ...
    const aggregations = aggregateHealthMetrics(
      (alerts ?? []) as OpsAlertRow[],
...
function aggregateHealthMetrics(
  ...
  for (const metric of metrics) {
    const key = `${metric.app ?? "unknown"}:${metric.env ?? "unknown"}:${
      metric.event ?? "unknown"
    }`;
    if (!groups.has(key)) {
      groups.set(key, {
        app: metric.app ?? null,
  ...
  for (const alert of alerts) {
    const key = `${alert.app ?? "unknown"}:${alert.env ?? "unknown"}:${
      alert.event ?? "unknown"
    }`;
    if (!groups.has(key)) {
      groups.set(key, {
  ...
  for (const [, group] of groups) {
    ...
    const totalRequests = group.metrics.reduce(
      (sum, m) => sum + (m.total || 0),
      0,
    );
    const totalFailures = group.metrics.reduce(
      (sum, m) => sum + (m.total || 0) * (m.failure_rate || 0),
      0,
    );
