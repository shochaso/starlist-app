...
serve(async (req) => {
  ...
  try {
    ...
    const expiresIn = Math.min(Math.max(body.expiresIn ?? 600, 60), 3600); // 60s〜3600s
    ...
    const sign = async (path: string) => {
      ...
      if (error || !data?.signedUrl) {
        return json({ error: "Sign error" }, 500, req);
      }
    ...
    if (body.mode === "path") {
      ...
      const obj = objs[0] as StorageObject;
    ...
    if (body.mode === "asset") {
      ...
      if (!Number.isFinite(assetId)) {
        return json({ error: "Bad assetId" }, 400, req);
      }
      ...
      const a = assets[0] as MediaAsset;
      if (a.bucket !== "private-originals") {
        return json({ error: "Only private-originals allowed" }, 403, req);
      }
