...
2. **ワークフローファイル確認**: `workflow_dispatch`が含まれていることを確認
   ...
   - **mainブランチ**: ❌ `workflow_dispatch`なし（PR #39マージ後に反映）

3. **競合解消**: ワークフローファイルはhotfixブランチのバージョン（`workflow_dispatch`含む）を採用
...
1. **PR #39をマージ**: GitHub UIから **Squash and merge** を実行
   - URL: https://github.com/shochaso/starlist-app/pull/39
   - **重要**: ワークフローファイル（`weekly-routine.yml`、`allowlist-sweep.yml`）に`workflow_dispatch`が含まれていることを確認
