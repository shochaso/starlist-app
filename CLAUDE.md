...
- **テーマ**: theme_provider_enhanced.dart
...
### 重要な注意事項
- **フォロー機能は削除済み**: follow_screen.dart、fan_following_stars.dart等は完全に削除
- **YouTube履歴**: スター名とジャンルを表示、動画名・YouTuber名は非表示
- **データ取り込み**: showAppBar=falseでメイン画面に埋め込み表示
- **マイリスト**: 5タブ構成（フォロー中、お気に入り、コンテンツ、プレイリスト、最近の活動）
- **テストコマンド**: `flutter run -d chrome` でChrome実行

## UI/UXデザイン仕様

### 1. ホーム画面構成
- 上部ナビゲーション：ロゴ、検索、通知、プロフィール
- 中央コンテンツエリア：最新のスター投稿、おすすめコンテンツ
- 下部タブナビゲーション：ホーム、検索、データ取り込み、マイリスト、プロフィール
- 探す（横スクロールのカテゴリーアイコン）

### 2. カテゴリページ
- 画面上部に横スクロールするカテゴリータブ
- サブカテゴリの横スクロールフィルターチップ
- 2列グリッドでスターを表示

### 3. スター詳細ページ
- プロフィールヘッダー
- データタブセクション（視聴履歴、音楽、映像、購入、アプリ、食事）
- コンテンツ表示エリア

### 4. データ取り込み画面
- メインサービス6種類（YouTube、Spotify、Netflix、レシート、アプリ、Amazon）
- OCRスクリーンショット取り込み機能
- その他サービス3種類（その他SNS、その他動画、その他Shopping）
- 横スクロール・コンパクトレイアウトで「その他」系を配置
...
[
  {
    ...
    "rank": "プラチナ",
    "followers": 56000,
    "imageUrl": "https://example.com/star1.jpg"
  },
  {
    "id": "star2",
    "name": "水溜りボンド",
    ...
    "rank": "スーパー",
    "followers": 235000,
    "imageUrl": "https://example.com/star2.jpg"
  },
  {
    "id": "star3",
    "name": "板倉の健ちゃん",
    ...
    "rank": "レギュラー",
    "followers": 8700,
    "imageUrl": "https://example.com/star3.jpg"
  }
]
```
