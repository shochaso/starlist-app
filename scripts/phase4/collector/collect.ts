...
export async function collectRun(runId: string, baseDir = 'docs/reports/2025-11-14', dryRun = true): Promise<CollectResult> {
  ...
  try {
    ...
    if (!fs.existsSync(artifactPath)) {
      // If an environment-provided artifact map exists, try to download
      const artifactMapPath = process.env.PHASE4_ARTIFACT_MAP || '';
      let urlForRun: string | undefined;
      if (artifactMapPath && fs.existsSync(artifactMapPath)) {
        try {
          const mapping = JSON.parse(fs.readFileSync(artifactMapPath, 'utf8'));
          urlForRun = mapping?.[runId];
        } catch {
          urlForRun = undefined;
        }
      }
      ...
      if (urlForRun) {
        try {
          await downloadWithRetry(urlForRun, artifactPath, { timeoutMs: Number(process.env.PHASE4_DOWNLOAD_TIMEOUT_MS || 15000) });
          console.info(JSON.stringify({ event: 'artifactDownloaded', runId, url: urlForRun, path: artifactPath, ts: nowIso() }));
          // wait briefly for file system to settle if needed
          const maxWait = Number(process.env.PHASE4_DOWNLOAD_SETTLE_MS || 500);
          const start = Date.now();
          while (!fs.existsSync(artifactPath) && Date.now() - start < maxWait) {
            await new Promise(r => setTimeout(r, 10));
          }
          if (!fs.existsSync(artifactPath)) throw new Error('downloaded file missing after download');
        } catch (e: any) {
          console.info(JSON.stringify({ event: 'artifactDownloadFailed', runId, url: urlForRun, error: String(e?.message ?? e), ts: nowIso() }));
          // fallback to fixture
        ...
        // no url available, create fixture
        fs.writeFileSync(artifactPath, 'collector-fetched-fixture', 'utf8');
        console.info(JSON.stringify({ event: 'artifactFetchedFixture', runId, path: artifactPath, ts: nowIso() }));
      }
    }

    const computed = await computeSha256(artifactPath);
    let reportedSha = '';
    ...
    try {
      ...
      try { await writeMaskedSlackExcerpt(runId, `runsSummary update failed: ${String(e?.message ?? e)}`, path.join(baseDir, 'slack_excerpts')); } catch {}
      throw e;
    }
    ...
    try { await writeMaskedSlackExcerpt(runId, `error collecting run ${runId}: ${String(err?.message ?? '')}`, path.join(baseDir, 'slack_excerpts')); } catch {}
...
async function downloadWithRetry(urlStr: string, destPath: string, opts: { timeoutMs?: number, maxRedirects?: number } = {}): Promise<void> {
  const timeoutMs = opts.timeoutMs ?? 15000;
  const maxRedirects = opts.maxRedirects ?? 5;

  return retry(async () => {
    await downloadOnce(urlStr, destPath, timeoutMs, maxRedirects);
  }, { maxRetries: 2, initialDelayMs: 5000, backoffMultiplier: 2, nonRetryableCodes: [422,403] });
}

function downloadOnce(urlStr: string, destPath: string, timeoutMs: number, maxRedirects: number): Promise<void> {
  return new Promise((resolve, reject) => {
    const url = new URL(urlStr);
    if (url.protocol === 'file:') {
      try {
        const src = decodeURIComponent(url.pathname);
    ...
    const req = lib.get(urlStr, { timeout: timeoutMs }, (res) => {
      ...
      if (res.statusCode && (res.statusCode >= 300 && res.statusCode < 400) && res.headers.location && maxRedirects > 0) {
        res.resume();
        return downloadOnce(res.headers.location, destPath, timeoutMs, maxRedirects - 1).then(resolve).catch(reject);
      }
