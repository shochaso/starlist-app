...
git commit --no-verify -m "chore(ui): LIN-001 wording tweak" || {
...
DoD: PR→In Progress, Review→In Review, Merge→Done" \
  --base "${BASE_BRANCH}" 2>&1 | grep -oP 'https://github.com/[^/]+/[^/]+/pull/\d+' | head -1 | grep -oP '\d+$' || {
  echo "⚠️  PR作成に失敗しました。GitHub CLIの認証を確認してください。"
  exit 1
})

PR_URL="https://github.com/$(git config --get remote.origin.url | sed 's/.*github.com[:/]\([^.]*\).*/\1/')/pull/${PR_NUMBER}"
echo "✅ PR作成完了: ${PR_URL}"

# 自分にレビュー依頼
echo "6. レビュー依頼..."
gh pr edit "${PR_NUMBER}" --add-reviewer @me 2>/dev/null || {
